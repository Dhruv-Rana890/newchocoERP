...
class SaleController extends Controller
{
    ...
    public function productSaleData($id)
    {
        ...
        $product_sale = [];
        $all_product_sales = Product_Sale::where('sale_id', $id)->get();
        
        foreach ($rows as $key => $row) {
            $product_sale_data = $row['product_sale'];
            $item = $row['item'];
            
            // Get parent product name only (without child products in name)
            $name_with_code = $item->name . ' [' . ($item->code ?? '') . ']';
            ...
            if (!$product_sale_data->warehouse_store_product_id && $product_sale_data->variant_id) {
                ...
                if ($lims_product_variant_data) {
                    $name_with_code = $item->name . ' [' . $lims_product_variant_data->item_code . ']';
                }
            }
