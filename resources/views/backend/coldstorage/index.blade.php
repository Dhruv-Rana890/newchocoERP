    ...
    <section>
        <div class="container-fluid">
            ...
            @can('coldstorages-add')
                <a href="{{ route('coldstorages.create') }}" class="btn btn-info add-coldstorage-btn btn-icon"><i
                        class="dripicons-plus"></i> {{ __('db.add_cold_storage') }}</a>
            @endcan

            <button type="button" class="btn btn-warning btn-icon" id="toggle-filter">
                <i class="dripicons-experiment"></i> {{ __('db.Filter Cold Storages') }}
            </button>
        ...
        <div class="table-responsive">
            <table id="coldstorage-data-table" class="table pt-0" style="width: 100%">
                <thead>
                    <tr>
...
@push('scripts')
    <script>
        $("ul#coldstorage").siblings('a').attr('aria-expanded', 'true');
        $("ul#coldstorage").addClass("show");
        $("ul#coldstorage #coldstorage-list-menu").addClass("active");
        ...
        $(document).ready(function() {
            var table = $('#coldstorage-data-table').DataTable({
                responsive: true,
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('coldstorages.coldstorage-data') }}",
                    data: function(d) {
